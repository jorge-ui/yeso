...
(function () {
  var startDate = new Date('2022-08-24t13:30');
  var endDate = new Date('2022-09-12t13:30');
  var fullRangeDuration = (0, _dateFns.intervalToDuration)({
    start: startDate,
    end: endDate
  });
  var fullHrs = fullRangeDuration.days * 24 + fullRangeDuration.hours;
  var fullSecs = fullRangeDuration.days * 24 * 60 * 60 + fullRangeDuration.hours * 60 * 60 + fullRangeDuration.minutes * 60 + fullRangeDuration.seconds;

  function updateDate() {
    var today = new Date();
    var duration = (0, _dateFns.intervalToDuration)({
      start: today,
      end: endDate
    });
    var hrsRemaining = duration.days * 24 + duration.hours;

    var _format$split = (0, _dateFns.format)(new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, duration.minutes, duration.seconds), 'mm:ss').split(':'),
        _format$split2 = _slicedToArray(_format$split, 2),
        minsRemaining = _format$split2[0],
        secsRemaining = _format$split2[1];

    var yesoDiv = document.getElementById('yeso-div');
    yesoDiv.innerHTML = getTimeDiv(hrsRemaining, minsRemaining, secsRemaining);
    var hrsCovered = fullHrs - hrsRemaining;
    var fullSecsRemaining = duration.days * 24 * 60 * 60 + duration.hours * 60 * 60 + duration.minutes * 60 + duration.seconds;
    var coveredSecs = fullSecs - fullSecsRemaining;
    var coveredPercent = "".concat((coveredSecs / fullSecs * 100).toFixed(2), "%");
...
function getTimeDiv(hrs) {
  var mins = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "";
  var secs = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "";
  var className = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "";
  return "\n       <div class=\"time-div ".concat(className, "\">\n        <span>\n            ").concat(hrs, "\n            ").concat(!!mins.length ? ": ".concat(mins, " :") : "", "\n        <span/>\n        ").concat(secs.length ? "<span class=\"t-secs\">".concat(secs, "</span>") : "", "\n       </div>\n    ").replace(/[\n\t ]/, "");
}
